...
### Added
- `RidgeLine` series kind: `Series.ridgeLine yBaseline values` takes a y-axis position and raw float values; KDE is computed via Silverman's rule using `CommonMath.gaussianKde`; each group renders as a filled + stroked density curve raised off its baseline; curves overlap naturally (ridgeline effect); bandwidth is multiplied by `withPointRadius` (default 1.0); layering follows series order
- `Hexbin` series kind: `Series.hexbin radius points` takes a data-space hex radius and raw `(x, y)` pairs; points are binned into a flat-top hexagonal grid via `CommonMath.hexbinBins`; each occupied bin renders as a hexagon colored by count using the heatmap color scale (or `withColorScale`); empty bins are invisible; the right-margin color ramp is shared with the `Heatmap` renderer
- `BulletRange` and `BulletData` types and `Bullet` series kind: `Series.bullet` takes a `BulletData list` and renders horizontally stacked bullet rows; range bands use increasing opacity (light = poor, dark = good); actual value is a narrow foreground bar; target is a short vertical tick; row labels drawn to the left; axes suppressed automatically
